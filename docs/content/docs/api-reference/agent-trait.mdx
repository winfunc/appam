...
Each `run(...)` call starts from a fresh `initial_messages(...)` transcript for
the supplied prompt. Use `continue_session(...)` for persisted continuation or
`default_run_streaming_with_messages(...)` when your application owns an
...
Returns a one-shot `StreamBuilder` that allows registering closure handlers for
content, reasoning, tool calls, errors, and completion events. See
[StreamBuilder](/docs/api-reference/stream-builder) for details.
