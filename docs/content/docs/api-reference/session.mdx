...
For a single-process terminal chat, you do not need SQLite history. Keep the
latest `Session.messages`, append the next user prompt, and run the next turn
with `default_run_streaming_with_messages(...)`:
...
let input = "What did I ask first?";
...
Only update your retained messages after a successful turn. That keeps the last
known-good transcript intact if a provider request or tool call fails.
