...
- Exact `gpt-5.6-sol` and `gpt-daybreak-blue-latest` accept an explicit `XHigh`
  request while retaining a model-specific default of `High`; neither supports
  sampling parameters, and unknown suffixed variants remain unsupported for
  `XHigh`.
- `ReasoningEffort::None` is a first-class value and is required for some GPT-5.5 sampling combinations.
