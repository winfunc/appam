...
`StreamBuilder` provides an ergonomic, closure-based API for handling streaming
events without implementing the `StreamConsumer` trait. It is returned by
`agent.stream("prompt")` on `RuntimeAgent` and uses a fluent builder pattern
where you chain `.on_*` handlers and finalize with `.run()`.

`StreamBuilder` is a one-shot convenience for one prompt. It returns a
populated `Session`, but it does not store hidden conversation state inside the
agent. For readline-style multi-turn loops, keep `Session.messages` and pass
the updated transcript to `default_run_streaming_with_messages(...)`.
...
Use `StreamBuilder` for quick, inline streaming of a single turn. Use
`StreamConsumer` when you need reusable consumers or want to broadcast to
multiple destinations. Use
`appam::agent::runtime::default_run_streaming_with_messages(...)` when your
application owns and replays a multi-turn transcript.
