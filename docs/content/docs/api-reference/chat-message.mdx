...
| `provider_response_id` | `Option<String>` | Provider-native response ID used by APIs that support cross-turn continuation |
...
```rust
let message = ChatMessage::user("What changed in this file?");
```

`ChatMessage::user(...)` creates a plain user text message with the optional
tool, reasoning, provider, and status fields left empty. It is the shortest way
to append a follow-up prompt when you are manually replaying `Session.messages`
in an in-memory chat loop.
