...
The most ergonomic way to handle a one-shot streamed turn is the closure-based
`StreamBuilder`, accessed via `agent.stream()`:
...
`agent.stream(prompt).run()` starts from that prompt each time. It does not
store hidden chat state inside the agent. For a readline loop, keep the returned
`Session.messages` and pass the updated transcript into the next turn.

## Interactive Multi-Turn Loops

Use `default_run_streaming_with_messages(...)` when your application already
owns the conversation transcript:
...
The coding-agent examples use this pattern so each file stays standalone while
still preserving prior user turns, assistant messages, tool calls, and tool
results.
