...
When `OPENAI_CODEX_ACCESS_TOKEN` is unset, the provider uses the sticky
round-robin pool in `OPENAI_CODEX_AUTH_FILES` when configured. Otherwise it
refreshes the local auth cache in `OPENAI_CODEX_AUTH_FILE` (default:
`~/.appam/auth.json`).
