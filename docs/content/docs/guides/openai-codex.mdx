...
The exact `gpt-5.6-sol` model supports `xhigh` reasoning on the Codex
subscription transport. Appam preserves both the model identifier and the
`xhigh` effort in the request; unknown suffixed variants continue to use the
fail-closed generic capability rules.
