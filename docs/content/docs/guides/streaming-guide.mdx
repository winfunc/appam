...
1. **StreamBuilder (closure-based)** -- Inline event handlers with a fluent API. Best for one-shot streamed turns.
...
`StreamBuilder` does not keep hidden state between calls. If you call
`.stream(input).run()` inside a readline loop, each call starts a fresh
`system + user` transcript. Keep `session.messages` yourself when you want an
in-memory multi-turn chat.

## In-Memory Conversation Replay

The coding-agent examples use this compact pattern:
...
Use this when your process owns the active chat. Use `continue_session(...)`
instead when the transcript should be loaded from SQLite-backed session history.
