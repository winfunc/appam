...
When you call `.stream().run()`, Appam runs a tool loop for that one user
prompt:
...
For a readline-style chat loop across multiple user prompts, keep the returned
`Session.messages` and pass the updated transcript into
`default_run_streaming_with_messages(...)` on the next prompt.
