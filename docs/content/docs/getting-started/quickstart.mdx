...
The `.stream()` method returns a `StreamBuilder` that lets you attach closures
to handle events as they arrive. `.run()` executes one agent turn and returns a
`Session` with conversation metadata. If you build an interactive loop, retain
`Session.messages` and replay them with the next user prompt instead of calling
`.stream(input)` as a fresh session each time.
