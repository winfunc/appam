...
- The loop keeps `Option<Vec<ChatMessage>>`, uses `agent.initial_messages(input)?` on the first turn, appends follow-ups with `ChatMessage::user(input)`, and records `session.messages` after successful turns.
- The response is streamed with `default_run_streaming_with_messages(...)` and `ConsoleConsumer::new()`, so content, reasoning, tool calls, and tool results appear in the terminal.
