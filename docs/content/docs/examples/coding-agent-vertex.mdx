...
- The response is streamed with `default_run_streaming_with_messages(...)` and `ConsoleConsumer::new()`, so normal content, reasoning content, tool calls, and tool results appear in the terminal.
