...
- Model `OPENAI_CODEX_MODEL` or `gpt-5.5`
...
- Otherwise it reads the auth-file pool from `OPENAI_CODEX_AUTH_FILES`, or the
  single cache in `OPENAI_CODEX_AUTH_FILE` (default: `~/.appam/auth.json`).
- If neither source is usable, the example opens the ChatGPT OAuth browser flow
  and stores the resulting credential in the first configured auth file before
...
- The readline loop keeps `Option<Vec<ChatMessage>>`, uses
  `agent.initial_messages(input)?` on the first turn, appends follow-ups with
  `ChatMessage::user(input)`, and records `session.messages` after successful
  turns.
- The response is streamed with `default_run_streaming_with_messages(...)` and
  `ConsoleConsumer::new()`, so reasoning, tool calls, and tool results are
  streamed to the console during each turn.
