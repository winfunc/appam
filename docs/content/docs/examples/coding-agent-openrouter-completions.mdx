...
- The response is streamed with `default_run_streaming_with_messages(...)` and `ConsoleConsumer::new()`, so reasoning content, tool calls, and tool results are streamed to the console during each turn.
