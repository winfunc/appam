...
pub struct AggregatedUsage {
    ...
    /// Total input tokens consumed by server-side compaction passes.
    ///
    /// Reported separately by providers (Anthropic `usage.iterations`) and
    /// excluded from `total_input_tokens`; billed at normal input rates.
    #[serde(default)]
    pub total_compaction_input_tokens: u64,

    /// Total output tokens generated by server-side compaction passes.
    #[serde(default)]
    pub total_compaction_output_tokens: u64,
...
impl AggregatedUsage {
    ...
    pub fn add_usage(&mut self, usage: &UnifiedUsage, provider: &str, model: &str) {
        ...
        if let Some(compaction_input) = usage.compaction_input_tokens {
            self.total_compaction_input_tokens += compaction_input as u64;
        }

        if let Some(compaction_output) = usage.compaction_output_tokens {
            self.total_compaction_output_tokens += compaction_output as u64;
        }
    ...
    pub fn format_detailed(&self) -> String {
        ...
        if self.total_compaction_input_tokens > 0 || self.total_compaction_output_tokens > 0 {
            lines.push(format!(
                "Compaction:        {} in / {} out",
                self.total_compaction_input_tokens, self.total_compaction_output_tokens
            ));
        }
...
impl UsageTracker {
    ...
    pub fn merge_aggregated(&self, other: &AggregatedUsage) {
        ...
        inner.total_compaction_input_tokens += other.total_compaction_input_tokens;
        inner.total_compaction_output_tokens += other.total_compaction_output_tokens;
