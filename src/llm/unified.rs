...
pub enum UnifiedContentBlock {
    ...
    /// Server-side context compaction artifact.
    ///
    /// Emitted by providers that support server-side compaction when the
    /// conversation crosses the configured token threshold. The block must be
    /// replayed verbatim on subsequent requests so the provider can continue
    /// from the compacted context: Anthropic ignores all content before the
    /// last compaction block, and OpenAI resumes from the encrypted summary.
    ///
    /// # Provider Differences
    ///
    /// - **Anthropic**: `content` carries the human-readable summary text.
    ///   `content` may be absent when the internal summarization step failed
    ///   (e.g., the model attempted a tool call instead of summarizing).
    /// - **OpenAI Responses**: `encrypted_content` carries an opaque payload
    ///   that is not human-readable; `id` is the provider item id (`cmp_...`).
    ///
    /// Blocks are only meaningful to the provider that produced them. When a
    /// session switches providers, compaction blocks from the previous
    /// provider are skipped during conversion and the full retained history
    /// is replayed instead.
    Compaction {
        /// Human-readable summary text (Anthropic).
        #[serde(default, skip_serializing_if = "Option::is_none")]
        content: Option<String>,
        /// Opaque encrypted summary payload (OpenAI Responses).
        #[serde(default, skip_serializing_if = "Option::is_none")]
        encrypted_content: Option<String>,
        /// Provider-native item identifier, when exposed.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        id: Option<String>,
    },
...
pub struct UnifiedUsage {
    ...
    /// Input tokens consumed by server-side compaction passes (Anthropic).
    ///
    /// Anthropic's `usage.iterations` reports compaction passes separately
    /// and *excludes* them from the top-level `input_tokens`/`output_tokens`.
    /// These fields carry that extra billed usage so cost accounting stays
    /// accurate without distorting the per-request context-size signal.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_input_tokens: Option<u32>,
    /// Output tokens generated by server-side compaction passes (Anthropic).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_output_tokens: Option<u32>,
}
