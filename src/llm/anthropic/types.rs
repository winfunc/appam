...
pub enum ContentBlock {
    ...
    /// Server-side compaction summary (assistant only, beta: compact-2026-01-12).
    ///
    /// Returned at the start of the assistant response when server-side
    /// compaction triggers. The block must be replayed verbatim on subsequent
    /// requests; the API ignores all content blocks before the last
    /// `compaction` block in the prompt.
    ///
    /// `content` may be `null` when the internal summarization step failed
    /// (e.g., the model attempted a tool call instead of writing a summary).
    Compaction {
        /// Summary text produced by the compaction pass.
        #[serde(default)]
        content: Option<String>,
        /// Optional cache control (a cache breakpoint may be placed on a
        /// compaction block when it is replayed).
        #[serde(skip_serializing_if = "Option::is_none")]
        cache_control: Option<CacheControl>,
    },
}
...
pub struct Usage {
    ...
    /// Per-iteration usage breakdown (beta: compact-2026-01-12).
    ///
    /// Present on every response once the compaction beta is enabled. The
    /// top-level `input_tokens`/`output_tokens` EXCLUDE `compaction`
    /// iterations; the total billed usage is the sum across all entries.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub iterations: Option<Vec<UsageIteration>>,
}

/// One sampling iteration inside a single API request (beta: compact-2026-01-12).
///
/// With server-side compaction enabled a request may perform multiple
/// sampling passes: `compaction` entries describe the internal summarization
/// pass over the pre-compaction context, and `message` entries describe the
/// normal response generation. Cost accounting must sum all iterations.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UsageIteration {
    /// Iteration kind: `"compaction"` or `"message"`.
    #[serde(rename = "type")]
    pub iteration_type: String,

    /// Input tokens consumed by this iteration.
    #[serde(default)]
    pub input_tokens: u32,

    /// Output tokens generated by this iteration.
    #[serde(default)]
    pub output_tokens: u32,
}

impl Usage {
    /// Sum the input/output tokens of `compaction` iterations, when present.
    ///
    /// Returns `None` when no compaction pass ran during the request so the
    /// unified usage keeps its compact wire representation.
    pub fn compaction_token_totals(&self) -> Option<(u32, u32)> {
        let iterations = self.iterations.as_ref()?;
        let (input, output) = iterations
            .iter()
            .filter(|iteration| iteration.iteration_type == "compaction")
            .fold((0u32, 0u32), |(input, output), iteration| {
                (
                    input.saturating_add(iteration.input_tokens),
                    output.saturating_add(iteration.output_tokens),
                )
            });
...
mod tests {
    ...
    #[test]
    fn test_compaction_content_block_roundtrip() {
        // API-shaped payload with a summary
        let block: ContentBlock = serde_json::from_value(json!({
            "type": "compaction",
            "content": "Summary of the conversation so far."
        }))
        .expect("compaction block should parse");
        assert!(matches!(
            &block,
            ContentBlock::Compaction { content: Some(content), .. }
                if content == "Summary of the conversation so far."
        ));

        // Failed summarization returns content: null and must still parse
        let failed: ContentBlock = serde_json::from_value(json!({
            "type": "compaction",
            "content": null
        }))
        .expect("null-content compaction block should parse");
        assert!(matches!(
            failed,
            ContentBlock::Compaction { content: None, .. }
        ));

        // Replay serialization keeps the wire tag
        let json = serde_json::to_value(&block).expect("block should serialize");
        assert_eq!(json["type"], "compaction");
        assert_eq!(json["content"], "Summary of the conversation so far.");
    }

    #[test]
    fn test_usage_iterations_parse_and_totals() {
        ...
        .expect("usage with iterations should parse");

        assert_eq!(usage.compaction_token_totals(), Some((180_000, 3_500)));

        // Message-only iterations (beta enabled, no compaction this turn)
        let no_compaction: Usage = serde_json::from_value(json!({
            "input_tokens": 23000,
            "output_tokens": 1000,
            "iterations": [
                { "type": "message", "input_tokens": 23000, "output_tokens": 1000 }
        ...
        assert_eq!(no_compaction.compaction_token_totals(), None);
    }
